...
configure_alacritty_windows() {
  local dest_dir dest zsh_win distro cfg_dir wsl_home
  distro="$(wsl_distro)"
  dest_dir="$(cygpath -u "$APPDATA")/alacritty"
  dest="$dest_dir/alacritty.toml"
  zsh_win="$(cygpath -m /usr/bin/zsh.exe)"
  cfg_dir="$(cygpath -m "$DOTFILES_DIR")/.config/alacritty"
  if [[ -n "$distro" ]]; then
    # wslpath prints \\wsl.localhost\<distro>\home\<user>; Win32 and TOML both take forward slashes
    wsl_home="$(wsl.exe -d "$distro" -- wslpath -w '~' 2>/dev/null | tr -d '\r\0' | tr '\\' '/')"
    if [[ -n "$wsl_home" && -d "$wsl_home/.dotfiles/.config/alacritty" ]]; then
      cfg_dir="$wsl_home/.dotfiles/.config/alacritty"
    fi
  fi
  mkdir -p "$dest_dir"
  if [[ -f "$dest" ]] && ! grep -q "Generated by dotfiles install.sh" "$dest"; then
    warn "Backing up existing $dest -> $dest.bak"
    cp -f "$dest" "$dest.bak"
  fi
  {
    echo "# Generated by dotfiles install.sh. Re-run ~/.dotfiles/install.sh to regenerate. Do not edit by hand."
    echo "# Fonts, colors and key bindings come from the imported file; edit them in the dotfiles repo."
    echo
    echo "[general]"
    echo "import = [\"$cfg_dir/windows.toml\"]"
    echo
    echo "[env]"
    ...
    echo
    if [[ -n "$distro" ]]; then
      echo "# Default shell: WSL ($distro). To use the native Git Bash zsh instead, swap the two blocks."
      echo "[terminal.shell]"
      ...
      echo "# Windows shell: zsh from Git for Windows (login shell so /etc/profile sets up PATH)"
      echo "[terminal.shell]"
      echo "program = \"$zsh_win\""
      echo 'args = ["-l"]'
    fi
  } >"$dest"
  ok "Alacritty config written to $dest (imports $cfg_dir, shell: ${distro:-native zsh})"
}
